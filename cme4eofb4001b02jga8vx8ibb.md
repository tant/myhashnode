...
Agentic chunking là chiến lược tiên tiến và mang tính thử nghiệm nhất hiện. Nó sử dụng chính LLM như một agent thông minh để quyết định cách chia một tài liệu. Quá trình này thường bao gồm:

* **Propositionizing** (Agent tạo các mệnh đề): LLM đầu tiên phân rã văn bản nguồn thành một loạt các "mệnh đề"—những câu phát biểu độc lập, đơn lẻ. Ví dụ, "RAG kết hợp truy xuất và sinh văn bản" trở thành một mệnh đề.
    
* **Agentic grouping** (Agent gom nhóm): Một tác tử dựa trên LLM sau đó phân tích các mệnh đề này. Nó quyết định xem một mệnh đề mới có thuộc về chunk hiện tại hay không, hay nó đại diện cho một sự thay đổi chủ đề cần bắt đầu một chunk mới. Nó giống như một người đang đọc và quyết định, "Được rồi, phần tiếp theo là một ý mới, mình sẽ bắt đầu một đoạn mới ở đây."
    

Phương pháp này mô phỏng tư duy của con người trong quá trình chunking. Nó linh hoạt, bảo toàn ngữ cảnh và thậm chí có thể làm giàu các chunk bằng siêu dữ liệu như tóm tắt hoặc tiêu đề do chính agent tạo ra. Đây là đỉnh cao của sự mạch lạc ngữ nghĩa. Tuy nhiên, Bách nhận ra đây là một chiến lược 5 sao có lý do của nó. Nó mang tính thử nghiệm cao, cực kỳ tốn kém về mặt tính toán và phức tạp để triển khai. Nó đại diện cho tương lai nhưng có thể chưa thực tế cho hầu hết các hệ thống sản xuất ngày nay.
...
| **Chiến Lược** | **Cơ Chế Cốt Lõi** | **Ưu Điểm Chính** | **Nhược Điểm Chính** | **Trường Hợp Sử Dụng Lý Tưởng** |
| --- | --- | --- | --- | --- |
| **Fixed-size** | Chia văn bản thành các chunk có số lượng ký tự/token cố định. | Đơn giản, nhanh, có thể dự đoán, chi phí tính toán thấp. | Bỏ qua ngữ cảnh ngữ nghĩa, thường phá vỡ câu và ý tưởng, dẫn đến chất lượng truy xuất kém. | Văn bản đồng nhất, phi cấu trúc, nơi tốc độ quan trọng hơn độ chính xác (ví dụ: log đơn giản). Một phương pháp cơ sở nhanh chóng. |
| **Sentence splitting** | Chia văn bản dựa trên các dấu kết thúc câu. | Đảm bảo mỗi chunk là một ý tưởng hoàn chỉnh. Mạch lạc hơn về mặt ngữ nghĩa so với fixed-size. | Kích thước chunk không nhất quán. Một câu đơn lẻ thường thiếu ngữ cảnh đủ cho các truy vấn phức tạp. | Nội dung dạng ngắn hoặc dữ liệu hội thoại nơi các câu riêng lẻ mang ý nghĩa quan trọng. |
| **Recursive character** | Chia văn bản theo tầng bậc bằng một danh sách các dấu phân tách được ưu tiên (ví dụ: đoạn, dòng, từ). | Cân bằng tuyệt vời giữa hiệu suất và sự gắn kết ngữ nghĩa. Cố gắng giữ các văn bản liên quan lại với nhau. Là lựa chọn mặc định mạnh mẽ cho văn bản chung. | Vẫn có thể dẫn đến các lần chia tùy tiện nếu văn bản thiếu cấu trúc rõ ràng và kích thước chunk nhỏ. | Phương pháp nên dùng cho hầu hết các tài liệu văn bản phi cấu trúc (ví dụ: bài báo, báo cáo). |
| **Structural (markdown / layout-aware)** | Chia văn bản dựa trên cấu trúc sẵn có của tài liệu (ví dụ: tiêu đề markdown, bảng biểu pdf, danh sách). | Độ trung thực cao nhất với cấu trúc của tài liệu gốc. Bảo tồn hoàn hảo ngữ cảnh logic và trực quan. | Yêu cầu các bộ phân tích chuyên dụng. Chỉ áp dụng cho các tài liệu có cấu trúc rõ ràng, máy có thể đọc được. | Các tài liệu có cấu trúc cao như tài liệu kỹ thuật (markdown) hoặc các tệp pdf phức tạp (layout-aware). |
| **Semantic** | Sử dụng embedding câu để tìm các "điểm gãy" nơi chủ đề hoặc ý nghĩa của văn bản thay đổi. | Tạo ra các chunk rất mạch lạc, giàu ngữ cảnh dựa trên ý nghĩa, không phải quy tắc. Thích ứng với nội dung của văn bản. | Tốn kém về mặt tính toán. Các nghiên cứu gần đây cho thấy lợi ích về hiệu suất có thể không tương xứng với chi phí trong mọi trường hợp. | Văn bản dày đặc, dạng dài với các thay đổi chủ đề tinh vi, nơi sự mạch lạc ngữ nghĩa tối đa là quan trọng và chi phí không phải là rào cản lớn. |
| **Agentic** | Sử dụng LLM để phân tích văn bản, trích xuất các mệnh đề và quyết định ranh giới chunk. | Phương pháp nhận biết ngữ cảnh và "thông minh" nhất. Mô phỏng tư duy giống con người để tạo ra các chunk tối ưu. | Mang tính thử nghiệm cao, chi phí tính toán và độ trễ cực cao, phức tạp để triển khai và đưa vào sản xuất. | Các dự án r&d tiên phong; các tình huống đòi hỏi mức độ hiểu biết ngữ cảnh cao nhất tuyệt đối, bất kể chi phí. |